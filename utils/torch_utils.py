...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, updates=0):
        ...
        # 새 모델 인스턴스 생성 (원본 모델과 동일한 구조여야 함)
        # new_model = deepcopy(model.module if is_parallel(model) else model)
        # new_model.load_state_dict(model_state_dict)
        new_model = type(model)(showinfo=False)  # 모델의 같은 클래스의 새 인스턴스
        new_model.load_state_dict(model.state_dict())

        # 필요한 경우, 모델을 원래 모델과 동일한 device로 이동
