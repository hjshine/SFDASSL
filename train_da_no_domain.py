...
from models.yolo_original import Model
...
def fusion_weak_strong(bboxes, scores, clses, bboxes_s, scores_s, clses_s, per_batch_size, device, iou_thres, conf_thres, wbf_iou_thr):
    ...
    for img_id in range(per_batch_size):
        ...
        #boxes_, scores_, labels_ = apply_soft_nms(wbf_boxes, wbf_scores, wbf_labels, sigma=0.5, score_threshold=conf_thres, iou_threshold=iou_thres, method='linear')
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, ind in pbar:
            ...
            with amp.autocast(enabled=cuda):
                ...
                # features_t = grad_reverse(features)
                # D_img_out_t = domain_img(features_t)
                # loss_D_img_t = F.binary_cross_entropy_with_logits(D_img_out_t, torch.FloatTensor(D_img_out_t.data.size()).fill_(target_label).to(device))
                # loss_D_img_t = loss_D_img_t * opt.domain_weight
                ...
                # loss = loss + loss_D_img_t
                # loss_items = torch.cat((loss_items, loss_D_img_t.detach().unsqueeze(0)), 0)
    ...
    if rank in [-1, 0]:
        # Plots
        if plots:
            plot_results(save_dir=save_dir, distill=opt.distill_loss, cons=opt.consistency_loss, cont=opt.contrastive, domain=False)  # save as results.png
            if wandb_logger.wandb:
