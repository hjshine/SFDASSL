...
def ConsClsLoss(reference, alignment) :
        reference_prob = F.softmax(reference, dim=1)
        alignment_log_prob = F.log_softmax(alignment, dim=1)
        loss = F.kl_div(alignment_log_prob, reference_prob, reduction='batchmean')
